...
melted_cor_matrix_filtered <- melted_cor_matrix %>%
  filter((value >= 0.50 & value > 0) | (value <= -0.50 & value < 0))
...
# Eliminazione colonne con valore positivo >= 0.50, e valore negativo <= -0.50
dataset_finale <- filtered_numeric_cols %>%
  select(-FILENAME, -URL, -URLLength, -Domain, -DomainLength, -IsDomainIP, -URLSimilarityIndex, -HasObfuscation,
         -NoOfObfuscatedChar, -ObfuscationRatio, -NoOfLettersInURL, -LetterRatioInURL, -NoOfDegitsInURL, 
         -DegitRatioInURL, -NoOfEqualsInURL, -NoOfQMarkInURL, -NoOfOtherSpecialCharsInURL, 
         -SpacialCharRatioInURL, -IsHTTPS, -Title, -DomainTitleMatchScore, -IsResponsive, -NoOfURLRedirect,
         -HasDescription, -HasSocialNet, -HasSubmitButton, -HasHiddenFields, -HasCopyrightInfo,
...
ggplot(data = melted_cor_matrix_finale, aes(x = Var1, y = Var2, fill = value)) +
  ...
  labs(
    title = "Heatmap della Matrice di Correlazione (Dataset Finale con filtraggio1 (>= 0.50 e <= -0.50))",
    x = "Variabili",
    y = "Variabili"
  )
