...
data2 <- fread("Phishing_URL_Dataset_4.csv")
...
analyze_groups <- function(dataset, dataset_name) {
  ...
  for (col in columns) {
    ...
    # Boxplot e boxplot a intaglio per colonne numeriche
    ...
    # Istogramma per colonne numeriche
    if (is.numeric(dataset[[col]])) {
    ...
    # Grafico a torta per colonne categoriali (factor o character)
    if (is.factor(dataset[[col]]) || is.character(dataset[[col]])) {
      counts <- dataset %>% count(!!sym(col))
      png(filename = file.path(dataset_dir, paste0("piechart_", col, ".png")), width = 800, height = 600)
      p <- ggplot(counts, aes(x = "", y = n, fill = !!sym(col))) +
        geom_bar(stat = "identity", width = 1) +
        coord_polar("y") +
        labs(title = paste("Grafico a torta per", col), x = NULL, y = NULL) +
        theme_minimal()
      print(p)
      dev.off()
    }
...
# Analisi del dataset
analyze_groups(data2, "Phishing_URL_Dataset_4.csv")